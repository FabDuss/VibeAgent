...
**Status**: draft
**Delivery**: branch `main` ; commits none ; pushed: no ; PR: none ; deployed: no ; verified: no
**Lane**: standard
...
- D7 Prompt de `vibe` : agent par défaut, il reçoit toutes les sessions -> une question simple reçoit une réponse directe (pas de plan, pas de numéro) ; ce qui change le dépôt passe par une voie ; quand l'utilisateur a besoin d'une session standard, il donne la sortie propre à la surface (F2, F8) : pour soi dans ce projet `.claude/settings.local.json` = `{ "agent": "" }` ; pour une session CLI `claude --settings '{"agent":""}'` (bash / zsh) ou `'{\"agent\":\"\"}'` (Windows PowerShell 5.1) ; dans le cloud, aucune sortie hors modification du fichier commité. [change le comportement visible de vibe : soumis à l'utilisateur via le handback]
...
- INVARIANT à écrire quand WP01 est APPROVE :
  ...
  - INV-008 (D) fait externe : `agent` dans `.claude/settings.json` démarre chaque session CLI (y compris `-p`) sur cet agent ; une valeur vide (`--settings`, `settings.local.json`) rend le Claude Code standard ; le cloud ne lit que le fichier commité. Verified: 2026-09-23 (F1-F3, F8).
...
- Q4 D7 (vibe répond directement aux questions simples, sans plan, et indique comment sortir de vibe) : **recommandé : oui.** -- bloque WP03.
...
4. In `tests/check-kit.sh` section 6, capture each install's output, and for BOTH scripts assert: (a) fresh repo -> `.claude/settings.json` is byte-identical to `templates/claude/settings.json`, output has `default agent : vibe (.claude/settings.json)` and the step 5 line with `'claude' then starts as vibe` ; (b) the re-run after `tamper` (which now also appends `PROJECT EDIT` to `.claude/settings.json`) keeps it ; (c) repo whose `.gitignore` is `.claude/*` + `!.claude/agents/`, no settings file -> file created, output has a `WARNING` naming `.claude/settings.json` and no `WARNING` naming `.claude/agents/vibe.md` ; (d) pre-existing `{ "hooks": {} }` -> byte-identical, output has the `hint` line, the step 5 line with `claude --agent vibe`, and no line naming `.claude/settings.json is git-ignored` ; (e) pre-existing `{"agent": "other"}` + opt-out flag -> byte-identical, output has the `not vibe` line and no `skipped` line ; (f) opt-out flag, no file -> no `.claude/settings.json`, output has the `skipped` line. Scenarios may be chained in one repo.
5. Update the header comment of `tests/check-kit.sh` (item 6) to cover the settings file, its states and the opt-out.
...
- [unit] mutation check reported by the Codeur (then reverted): removing the settings copy from one script fails (a) ; forcing an overwrite fails (b)/(d) ; changing one state string in ONE script fails (a), (d) or (e) for that script.
...
**DoD**:
- [checks] `bash tests/check-kit.sh` green.
- [unit] `grep -rn "claude --agent vibe" README.md CLAUDE.md templates/CLAUDE.md .claude/agents install.sh install.ps1` -> every hit presents it as the explicit alternative or the not-default path ; none says it is the only way (reviewed by vibe).
- [unit] each surface named in the docs is tagged with its truth: CLI verified, cloud per docs, VS Code per H1 (reviewed by vibe).
- [live] NOT live-verified: the real proof of the prompt change is the first session where vibe is the default (STACK LIVE_CHECK) -- degradation: vibe opens a plan for a plain question ; cloud: first cloud session after the push.
